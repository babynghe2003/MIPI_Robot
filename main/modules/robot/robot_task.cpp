...
extern "C" void robot_update_task(void *pvParameters)
{
    ...
    ESP_LOGI(TAG, "MPU9250 initialized on channel 1");
    ...
    ret = tca9548a_select_channel(mux_handle, LEFT_CHANNEL);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to select AS5600 left channel: %s", esp_err_to_name(ret));
        vTaskDelete(NULL);
        return;
    ...
    while (1) {
        ...
        right = as5600_right.getSensorAngle();
        ...
        left = as5600_left.getSensorAngle();
