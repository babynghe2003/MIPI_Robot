...
// Robot task state definitions (copy from task_manager.h to avoid circular import)
typedef enum {
    ROBOT_STATE_INIT,              
...
BLDCDriver3PWM driverRight = BLDCDriver3PWM(RIGHT_MOTOR_CHANNEL_A, RIGHT_MOTOR_CHANNEL_B, RIGHT_MOTOR_CHANNEL_C);

#define OFF_SET 7

PIDController pid_stab_L{DEFAULT_KP, DEFAULT_KI, DEFAULT_KD, 100000, 50}; // High ramp = no rate limiting
PIDController pid_stab_R{DEFAULT_KP, DEFAULT_KI, DEFAULT_KD, 100000, 50}; 
PIDController pid_pos{DEFAULT_KP2, DEFAULT_KI2, DEFAULT_KD2, 1000, 7}; // Lower ramp for smoother velocity changes
PIDController pid_dir{DEFAULT_KP3, DEFAULT_KI3, DEFAULT_KD3, 1000, 1}; // Even lower ramp for direction control
...
static volatile bool g_logging_enabled = false; // display task prints only when enabled
...
extern "C" bool robot_pid1_set_kp(float v) {
    if (pid_param_mutex && xSemaphoreTake(pid_param_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
        pid_stab_L.P = v; pid_stab_R.P = v; xSemaphoreGive(pid_param_mutex); return true;
    }
    return false;
}
extern "C" bool robot_pid1_set_ki(float v) {
    if (pid_param_mutex && xSemaphoreTake(pid_param_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
        pid_stab_L.I = v; pid_stab_R.I = v; xSemaphoreGive(pid_param_mutex); return true;
    }
    return false;
}
extern "C" bool robot_pid1_set_kd(float v) {
    if (pid_param_mutex && xSemaphoreTake(pid_param_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
        pid_stab_L.D = v; pid_stab_R.D = v; xSemaphoreGive(pid_param_mutex); return true;
    }
    return false;
}
...
extern "C" bool robot_pid2_set_kp(float v) {
    if (pid_param_mutex && xSemaphoreTake(pid_param_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
        pid_pos.P = v; xSemaphoreGive(pid_param_mutex); return true;
    }
    return false;
}
extern "C" bool robot_pid2_set_ki(float v) {
    if (pid_param_mutex && xSemaphoreTake(pid_param_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
        pid_pos.I = v; xSemaphoreGive(pid_param_mutex); return true;
    }
    return false;
}
extern "C" bool robot_pid2_set_kd(float v) {
    if (pid_param_mutex && xSemaphoreTake(pid_param_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
        pid_pos.D = v; xSemaphoreGive(pid_param_mutex); return true;
    }
    return false;
}
...
extern "C" bool robot_pid3_set_kp(float v) {
    if (pid_param_mutex && xSemaphoreTake(pid_param_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
        pid_dir.P = v; xSemaphoreGive(pid_param_mutex); return true;
    }
    return false;
}
extern "C" bool robot_pid3_set_ki(float v) {
    if (pid_param_mutex && xSemaphoreTake(pid_param_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
        pid_dir.I = v; xSemaphoreGive(pid_param_mutex); return true;
    }
    return false;
}
extern "C" bool robot_pid3_set_kd(float v) {
    if (pid_param_mutex && xSemaphoreTake(pid_param_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
        pid_dir.D = v; xSemaphoreGive(pid_param_mutex); return true;
    }
    return false;
}

extern "C" void robot_set_target_leg_command(float v) { g_target_leg_command = v; }
extern "C" float robot_get_target_leg_command(void) { return g_target_leg_command; }
extern "C" bool robot_toggle_logging(void) { g_logging_enabled = !g_logging_enabled; return g_logging_enabled; }
...
extern "C" bool robot_motor_toggle(void) { g_bldc_enabled = !g_bldc_enabled; return g_bldc_enabled; }
...
static void boot_button_task(void *pv) {
    ...
    io_conf.pull_up_en = GPIO_PULLUP_ENABLE; // BOOT is typically pulled up, active-low when pressed
    gpio_config(&io_conf);
...
static esp_err_t init_motor_sensor_pair(int channel, CAS5600& sensor, BLDCMotor& motor, BLDCDriver3PWM& driver, const char* name)
{
    set_robot_state(ROBOT_STATE_INIT_MOTORS);
    esp_err_t ret = tca9548a_select_channel(mux_handle, channel);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to select %s channel: %s", name, esp_err_to_name(ret));
    ...
    motor.voltage_limit = 6; // Reduced from 12V to prevent MCPWM errors
    motor.init();
    motor.initFOC();
...
extern "C" void robot_update_task(void *pvParameters)
{
    ...
    if (init_motor_sensor_pair(RIGHT_CHANNEL, as5600_right, motorRight, driverRight, "AS5600 right") != ESP_OK) {
        set_robot_state(ROBOT_STATE_MOTOR_ERROR);
        vTaskDelete(NULL);
        return;
    }
    ...
    if (init_motor_sensor_pair(LEFT_CHANNEL, as5600_left, motorLeft, driverLeft, "AS5600 left") != ESP_OK) {
        set_robot_state(ROBOT_STATE_MOTOR_ERROR);
        vTaskDelete(NULL);
        return;
    }
    ...
    while (1) {
        read_sensor_data();
        caculate_pid();
        if (g_bldc_enabled && xSemaphoreTake(motor_mutex, pdMS_TO_TICKS(10)) == pdTRUE){
